...
export function registerOrderTools(server: McpServer) {
  server.tool(
    "create_order",
    "Create a new order for a user at a restaurant",
    {
    ...
    async ({ restaurantId, userId, items }) => {
      try {
        ...
        for (const item of items) {
          ...
          const unitPrice = menuData.price as number;
  ...
  server.tool(
    "get_active_order",
    "Get the active order for a user at a restaurant",
    {
      restaurantId: z.string(),
      userId: z.string(),
    },
  ...
  server.tool(
    "confirm_payment",
    "Confirm payment for the active order by submitting a payment receipt",
    {
      restaurantId: z.string(),
      userId: z.string(),
